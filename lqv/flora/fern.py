...
def add_tree_fern(x, y, scale=1.0):
    trunk_h = 2.8 * scale
    bpy.ops.mesh.primitive_cylinder_add(radius=0.12 * scale, depth=trunk_h, location=(x, y, trunk_h / 2))
    ...
    for i in range(6):
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(radius=0.4 * scale, location=(end_x, end_y, trunk_h + 0.1), subdivisions=2)

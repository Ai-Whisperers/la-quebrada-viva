// MapLibre ships a UMD bundle; load the global instead of ESM default-import.
// The maplibregl namespace provides Map / NavigationControl / ScaleControl / Popup as globals.
...
function loadMapLibre() {
  return new Promise((resolve, reject) => {
    ...
    s.onload = () => {
      try {
        // Pre-flight WebGL — the Maps SDK supports a graceful fallback via map.on('error').
        // Some sandboxed environments can't create a WebGL context; fall back gracefully.
        const probe = document.createElement('canvas');
        const gl = probe.getContext('webgl') || probe.getContext('experimental-webgl');
        if (!gl) return reject(new Error('WebGL not supported in this environment'));
      } catch (e) {
        return reject(e);
      }
...
async function buildMap() {
  ...
  map.on('load', async () => {
    ...
    await loadAndAdd('streams', 'hydrography_dem_v2.geojson', () => {
      map.addLayer({
        id: 'streams-line',
        type: 'line',
        source: 'streams-src',
    ...
    await loadAndAdd('roads_named', 'osm_roads_near.geojson', () => {
      map.addLayer({
        id: 'roads_named-line',
        type: 'line',
    ...
    await loadCsvThenAdd('gbif', 'gbif_-25.6073_-57.0355_30km.csv', (cells) => {
  ...
  // Apply URL params (set by index.html)
  if (window.__lqvInitialLayers && window.__lqvInitialLayers.length) {
    window.__lqvInitialLayers.forEach((name) => {
      const cb = document.querySelector(`[data-layer="${name}"]`);
      if (cb && !cb.checked) cb.click();
    });
  }

  // Reset button: fitBounds to LQV extent + restore all layers
  const resetBtn = document.getElementById('map-reset');
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      // Reset camera to LQV extent
...
buildMap().catch((err) => {
  console.error(err);
  // Sandbox / WebGL-disabled fallback: keep the static layer table visible
  // instead of a blank canvas. Loader replaced with action buttons + alternates.
  const splash = document.getElementById('splash');
  const splashStatus = document.getElementById('splash-status');
  const actions = document.getElementById('splash-actions');
  if (splash && splashStatus) {
    splash.classList.add('failed');
    splashStatus.innerHTML = 'Browser sandbox can\'t initialize WebGL.<br><small style="opacity:.7">The interactive map still works in any normal browser tab.</small>';
    if (actions) actions.style.display = 'flex';
  }
  document.querySelectorAll('[data-layer], .mode-toggle button').forEach((el) => {
    el.disabled = true; el.style.opacity = '.4'; el.title = 'WebGL required';
  });
  document.getElementById('stats').textContent = 'static-data fallback · WebGL unavailable';
});

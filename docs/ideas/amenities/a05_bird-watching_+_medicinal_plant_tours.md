# A05: Bird-watching + medicinal plant tours

**Category:** [Site Amenities & Experiences](../amenities/README.md)
**Priority:** P3
**Owner:** Wesley
**Status:** `planned`
**Quality:** `✓ reviewed`
**Source:** wes_recording_2026-06-30
**Deliverable:** trail map + guide training

---

## What Wes wants

trail map + guide training

**From Wes's words:**

> _"[Dream list 2026-06-30]: s | ⚪ Fase 2 | | D7 | **Romantische tafeltjes bij de quebrada** | Site-feature amenity | Erebus | ⚪ Fase 3 | | D8 | **VR met lokale dieren** (jaguar, vogels) voor site-context | Asset library: lokale fauna modellen | Erebus | 🟡 Al geleverd in render-pipeline | | D9 | **In 2030 draait het** — eerste Sonja-boeking voor haar 60ste | Project-mijlpaal | Allen | 📅 T-3.5 jaar | | D10 | **AI assistent weet all"_

> _"[Dream list 2026-06-30]: ebus + Wesley | Fase 2-3 | | S4 | Events — verjaardagen, familie-feesten, retreats | Erebus + Wesley | Fase 2-3 | | S5 | Eco-tourism — wandel-routes, vogelobservatie, medicinale planten-tour | Erebus + Wesley | Fase 3 | | S6 | VR-tour voor investeerders vóór de bouw | Erebus (3DGS pipeline) | Nu | | S7 | Lokaal personeel in dienst (Jos, Thijs, plus nieuwe medewerker) | Wesley + Sonja | Nu | | S8 | Koei"_

> _"[Dream list 2026-06-30]: ase 2-3 | | S4 | Events — verjaardagen, familie-feesten, retreats | Erebus + Wesley | Fase 2-3 | | S5 | Eco-tourism — wandel-routes, vogelobservatie, medicinale planten-tour | Erebus + Wesley | Fase 3 | | S6 | VR-tour voor investeerders vóór de bouw | Erebus (3DGS pipeline) | Nu | | S7 | Lokaal personeel in dienst (Jos, Thijs, plus nieuwe medewerker) | Wesley + Sonja | Nu | | S8 | Koeien overnemen van oude"_

## Why this matters



## Full picture — context, constraints, history

**Site amenities + experiences.** Phase 2-3 roadmap. Phase 1 should validate the basic stay experience before adding amenities. Amenities are what drive guest re-booking + event bookings + word-of-mouth.

## What we know already (research summary)

- **Deliverable target:** trail map + guide training
- **Source:** [Wes's brainstorm recording](../../audios/2026-06-30-wes-post-escritura/drafts/README.md) (raw transcript)
- **Cleaned version:** [dream list](../../audios/2026-06-30-wes-post-escritura/final/DREAMLIST_NL.md) + [action list](../../audios/2026-06-30-wes-post-escritura/final/ACTIONLIST_ES_EN.md)


## What needs research

- _No additional research tasks defined — see related ideas for context._

## Dependencies

_No explicit dependencies captured yet. Likely related to other ideas in this category — review the [INDEX.md](../../INDEX.md)._

## Risks & failure modes

_No specific risks identified beyond standard category risks. Add as they emerge._

## Cost / time estimate

Varies by amenity. Wellness pool: $5-15k. Weddings venue upgrade: $20-50k. Restaurant: $100-300k Phase 3.

## Done = shipped

**Acceptance criteria (measurable):**

- trail map + guide training
- Document the deliverable's location/path in this file
- Update the **Status** field above to `shipped`
- Add a 1-line entry to the Changelog below
- For research-type ideas: link the deliverable doc + note 1 key finding

## Recommended next action

1. **Wes: confirm Phase placement** (Phase 1/2/3 per the 2030 roadmap)
2. **Erebus: research reference properties** (Awasi, Chaa Creek, San Bernardino Estancia comparables)
3. **Wes: decide if concept art or build-spec needed first**

## Priority & status meaning

**Priority:** **Phase 3+ (year 2+) or nice-to-have.** Lowest priority.

**Status:** Scoped and queued. Not yet started.

## Sources & references

- [Wes's brainstorm recording (raw)](../../audios/2026-06-30-wes-post-escritura/drafts/README.md)
- [Wes's dream list (cleaned)](../../audios/2026-06-30-wes-post-escritura/final/DREAMLIST_NL.md)
- [Wes's action list (cleaned)](../../audios/2026-06-30-wes-post-escritura/final/ACTIONLIST_ES_EN.md)
- [Wes's capture brief](../../audios/2026-06-30-wes-post-escritura/README.md)
- [INDEX.md](../../INDEX.md) — master catalog
- [INSIGHTS.md](../INSIGHTS.md) — 20 patterns from reading the catalog
- [SUGGESTED.md](../SUGGESTED.md) — 20 ideas Erebus pushed in
- Related project docs: [HOUSING_PARK_CONCEPT.md](../../HOUSING_PARK_CONCEPT.md), [EUROPEAN_TOURISM_SPEC.md](../../EUROPEAN_TOURISM_SPEC.md), [MASTER_BRIEF.md](../../MASTER_BRIEF.md), [RESEARCH_GAPS.md](../../RESEARCH_GAPS.md)

## Changelog

- 2026-06-30: Idea created from consolidated session review (initial scaffold)
- 2026-06-30: Rich content pass — What/Why/Context/Research/Dependencies/Risks/Cost/Done/Action sections populated

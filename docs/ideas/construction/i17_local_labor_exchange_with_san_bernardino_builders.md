# I17: Local labor exchange with San Bernardino builders

**Category:** [Construction](../construction/README.md)
**Priority:** P2
**Owner:** Wesley+Erebus
**Status:** `planned`
**Quality:** `✓ reviewed`
**Source:** Erebus insights pass 2026-06-30
**Deliverable:** San Bernardino has a long tradition of German-Paraguayan builders who specialize in timber + cob. Partner with one for knowledge transfer + first build.

---

## What Wes wants

San Bernardino has a long tradition of German-Paraguayan builders who specialize in timber + cob. Partner with one for knowledge transfer + first build.

**From Wes's words:**

> _"[Action list 2026-06-30]: A1 | **Bamboe honing-kleur onderzoek** — kopen / groeien / importeren / termieten-behandeling | Web-research + leveranciers-vergelijking (Chaa Creek, San Bernardino Mennonieten) | Rapport + 3 offertes | | WA2 | **Bamboe-leveranciers in kaart brengen** | Database van Paraguayaanse bamboe-boeren | Lijst met contact | | WA3 | **Inboedel-onderzoek** — bedden, meubels, keuken — lokaal vs import (Troostwijk veilingen"_

## Why this matters

Wes is targeting San Bernardino's German community as his audience (per A06 / M02). Hiring a SB builder as the lead contractor simultaneously builds audience credibility, gets local knowledge, and demonstrates commitment to the community.

## Full picture — context, constraints, history

_This is a suggested idea from Erebus's insights pass 2026-06-30. It fills a gap in Wes's original 89 ideas. See [SUGGESTED.md](../../SUGGESTED.md) for the rationale._

See related existing ideas: [`A06`](../amenities/a06_dutch-german_restaurant_(san_bernardino_supply).md), [`C03`](../construction/c03_bamboo_supplier_map_(py).md)

## What we know already (research summary)



## What needs research

_Suggested addition — see [SUGGESTED.md](../../SUGGESTED.md) for context. Research tasks derived from gap analysis:_

- _No additional research tasks defined — see related ideas for context._

## Dependencies

**This idea depends on / is informed by:**

- [`A06`](../amenities/a06_dutch-german_restaurant_(san_bernardino_supply).md) — SB restaurant
- [`C03`](../construction/c03_bamboo_supplier_map_(py).md) — bamboo supplier



## Risks & failure modes

_No specific risks identified beyond standard category risks. Add as they emerge._

## Cost / time estimate

Same build cost as local contractors (or slightly higher for lead contractor role)

## Done = shipped

First house built by SB team + photo story on lqv-walkthrough.pages.dev

## Recommended next action

1. **Wes: review this suggestion** — accept / defer / reject with 1-line reason
2. **If accepted**: confirm priority (currently marked P2) + sprint slot
3. **Erebus: update status** in this file once Wes decides

## Priority & status meaning

**Priority:** **Phase 2 (months 9-18) or longer-term buildout.** Important but not blocking.

**Status:** Scoped and queued. Not yet started.

## Sources & references

- [Wes's brainstorm recording (raw)](../../briefs/wes_recording_2026-06-30_raw.md)
- [Wes's dream list (cleaned)](../../briefs/wes_recording_2026-06-30_dreamlist.md)
- [Wes's action list (cleaned)](../../briefs/wes_recording_2026-06-30_actionlist.md)
- [Wes's capture brief](../../briefs/wes_capture_brief.md)
- [INDEX.md](../../INDEX.md) — master catalog
- [INSIGHTS.md](../../INSIGHTS.md) — 20 patterns from reading the catalog
- [SUGGESTED.md](../../SUGGESTED.md) — 20 ideas Erebus pushed in
- Related project docs: [HOUSING_PARK_CONCEPT.md](../../HOUSING_PARK_CONCEPT.md), [EUROPEAN_TOURISM_SPEC.md](../../EUROPEAN_TOURISM_SPEC.md), [MASTER_BRIEF.md](../../MASTER_BRIEF.md), [RESEARCH_GAPS.md](../../RESEARCH_GAPS.md)

## Changelog

- 2026-06-30: Idea created from consolidated session review (initial scaffold)
- 2026-06-30: Rich content pass — What/Why/Context/Research/Dependencies/Risks/Cost/Done/Action sections populated

# C14: Gated vs open community (decision: open + minimal security)

**Category:** [Construction, Materials & Site Tech](../README.md)
**Priority:** P0
**Owner:** Wesley
**Status:** `decided`
**Source:** wes_recording_2026-06-30_v2
**Deliverable:** DECISION: open + gate + camera, not full gated
**Dependencies:** none

---

## Status meaning

Wesley has made a decision on the direction. Action items may still be in motion.

## Priority meaning

**Critical for the next sprint or the immediate escritura follow-up.** Blocks downstream work.

## Where this came from

Wes brainstorm recording, 2026-06-30. See [`../../briefs/wes_recording_2026-06-30_raw.md`](../../briefs/wes_recording_2026-06-30_raw.md).
Specifically from the second session (later in the day) where Wes covered legal/financial structure, operational research list, and house typology specs.

## Next action

- [x] Wes has decided ✅
- [ ] Erebus: reflect decision in code/plans as needed

## Risks / Open questions

_(empty — add as they come up)_

## Changelog

- 2026-06-30: Idea created from consolidated session review

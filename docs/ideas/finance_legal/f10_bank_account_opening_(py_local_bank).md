# F10: Bank account opening (PY local bank)

**Category:** [Finance, Legal & Insurance](../finance_legal/README.md)
**Priority:** P1
**Owner:** Wesley
**Status:** `in_progress`
**Quality:** `✓ reviewed`
**Source:** wes_recording_2026-06-30_v2
**Deliverable:** account opened

---

## What Wes wants

account opened

**From Wes's words:**

> _"[Action list 2026-06-30]: (Wesley-only, geen AI-actie) | # | Taak | Wanneer | Status | |---|---|---|---| | W1 | Interpol documenten afronden | Week 1 | ⚪ Persoonlijk | | W2 | Bankrekening openen (lokale bank PY) | Week 1-2 | ⚪ Persoonlijk | | W3 | Tekenen voor de grond (escritura) | ✅ 2026-06-27 gedaan | ✅ | | W4 | Google One 1 TB upgrade + foto-sync met Erebus | Vandaag | 🟡 Gestart (credit card nodig) | | W5 | Spaans leren ("_

> _"[Action list 2026-06-30]: Kiki over AI agency | Week 2-3 | ⚪ Sales | | W7 | Contract maken voor nieuwe medewerker (overname van oude) | Week 3-4 | 🟡 Via Sonja | | W8 | Bij de bank eruit halen (lening? investeerders-uitkering?) | Week 2-3 | ⚪ Persoonlijk | --- ## §2 — WESLEY + AI (korte termijn, AI doet het werk) | # | Taak | AI-actie | Resultaat | |---|---|---|---| | WA1 | **Bamboe honing-kleur onderzoek** — kopen / groeie"_

> _"[Action list 2026-06-30]: k voor investeerders** — wat, waarom, wanneer, ROI | Week 2-3 | Erebus + Wesley | | F3 | **BV's oprichten** — NL + PY structuur | Week 3-4 | Wesley + accountant | | F4 | **Investeerder-pitch sessies** plannen | Maand 2+ | Wesley | --- ## §7 — TECHNISCH (site + assets) | # | Taak | Wanneer | Owner | |---|---|---|---| | TE1 | **Prijzen-documenet maandelijks updaten** | Maandelijks | Erebus | | TE2 | **S"_

> _"[Action list 2026-06-30]: ley-only, geen AI-actie) | # | Taak | Wanneer | Status | |---|---|---|---| | W1 | Interpol documenten afronden | Week 1 | ⚪ Persoonlijk | | W2 | Bankrekening openen (lokale bank PY) | Week 1-2 | ⚪ Persoonlijk | | W3 | Tekenen voor de grond (escritura) | ✅ 2026-06-27 gedaan | ✅ | | W4 | Google One 1 TB upgrade + foto-sync met Erebus | Vandaag | 🟡 Gestart (credit card nodig) | | W5 | Spaans leren (Spaans-l"_

## Why this matters



## Full picture — context, constraints, history

**Finance, legal, insurance — the corporate spine.** F01 (4-entity BV cascade) is the single highest-leverage decision per [Insight #1](../INSIGHTS.md). Insurance + fire risk are coupled and urgent per [Insight #3](../INSIGHTS.md). Tax structuring (F11/F19) requires a NL+PY accountant.

## What we know already (research summary)

- **Deliverable target:** account opened
- **Source:** [Wes's brainstorm recording](../../audios/2026-06-30-wes-post-escritura/drafts/README.md) (raw transcript)
- **Cleaned version:** [dream list](../../audios/2026-06-30-wes-post-escritura/final/DREAMLIST_NL.md) + [action list](../../audios/2026-06-30-wes-post-escritura/final/ACTIONLIST_ES_EN.md)


## What needs research

- _No additional research tasks defined — see related ideas for context._

## Dependencies

**This idea depends on / is informed by:**

- [`F01`](f01_4-entity_bv_cascade_(land_py_+_finance_nl_+_phase_.md), [RESEARCH_GAPS.md](../../RESEARCH_GAPS.md)

## Changelog

- 2026-06-30: Idea created from consolidated session review (initial scaffold)
- 2026-06-30: Rich content pass — What/Why/Context/Research/Dependencies/Risks/Cost/Done/Action sections populated
../../research/strategy/RESEARCH_GAPS.md
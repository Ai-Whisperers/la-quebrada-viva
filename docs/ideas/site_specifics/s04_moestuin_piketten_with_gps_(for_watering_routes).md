# S04: Moestuin piketten with GPS (for watering routes)

**Category:** [Site-Specific Decisions & Constraints](../site_specifics/README.md)
**Priority:** P1
**Owner:** Wesley+Jos+Thijs
**Status:** `planned`
**Quality:** `✓ reviewed`
**Source:** wes_recording_2026-06-30
**Deliverable:** GPS map + first piketten placed

---

## What Wes wants

GPS map + first piketten placed

**From Wes's words:**

> _"[Action list 2026-06-30]: + site-visie) | Maand 1 | Wesley + Erebus | | T2 | **Depot-locatie bepalen** (opslag materialen op het terrein) | Maand 1 | Wesley + Thijs | | T3 | **Moestuin-piketten plaatsen met GPS** | Maand 1-2 | Wesley + Jos + Thijs | | T4 | **Water-afvoer locaties bepalen** | Maand 1-2 | Wesley + Thijs | | T5 | **Basis-gereedschap regelen** voor kleine werkzaamheden | Maand 1 | Wesley | | T6 | **Jos + Thijs onboard"_

> _"[Action list 2026-06-30]: sie) | Maand 1 | Wesley + Erebus | | T2 | **Depot-locatie bepalen** (opslag materialen op het terrein) | Maand 1 | Wesley + Thijs | | T3 | **Moestuin-piketten plaatsen met GPS** | Maand 1-2 | Wesley + Jos + Thijs | | T4 | **Water-afvoer locaties bepalen** | Maand 1-2 | Wesley + Thijs | | T5 | **Basis-gereedschap regelen** voor kleine werkzaamheden | Maand 1 | Wesley | | T6 | **Jos + Thijs onboarding**"_

> _"[Action list 2026-06-30]: y + Erebus | | T2 | **Depot-locatie bepalen** (opslag materialen op het terrein) | Maand 1 | Wesley + Thijs | | T3 | **Moestuin-piketten plaatsen met GPS** | Maand 1-2 | Wesley + Jos + Thijs | | T4 | **Water-afvoer locaties bepalen** | Maand 1-2 | Wesley + Thijs | | T5 | **Basis-gereedschap regelen** voor kleine werkzaamheden | Maand 1 | Wesley | | T6 | **Jos + Thijs onboarding** voor eerste maanden"_

> _"[Raw transcript 2026-06-30]: Actielijst maken." > > "Puitenwerksmede. Jolt en Ties voor de eerste maanden. Locatie vastbrengen van depot. Kijken voor plekjes." > > "Jolt en Ties moestuin. Piketten plaatsen met gps locatie. Waar beetjes lopen." > > "En belangrijke wateren afvoeren. Dat was eigenlijk voor wat ik hier nog allemaal kon doen." > > "Dan heb ik hier hoofdingen te regelen. Voor prijsberaming. Dat staat erbij." *(Concrete a"_

## Why this matters



## Full picture — context, constraints, history

**RV parcel-specific decisions.** 62-ha in Escobar, Paraguarí. Boundary + centroid documented in [docs/site_data/](../../site_data/). DEM + Sentinel-2 + GEDI canopy + OSM data integrated. Decision: open community (not gated) per [Insight #8](../INSIGHTS.md) + eco-framing matters per [Insight #8](../INSIGHTS.md).

## What we know already (research summary)

- **Deliverable target:** GPS map + first piketten placed
- **Source:** [Wes's brainstorm recording](../../audios/2026-06-30-wes-post-escritura/drafts/README.md) (raw transcript)
- **Cleaned version:** [dream list](../../audios/2026-06-30-wes-post-escritura/final/DREAMLIST_NL.md) + [action list](../../audios/2026-06-30-wes-post-escritura/final/ACTIONLIST_ES_EN.md)


## What needs research

- _No additional research tasks defined — see related ideas for context._

## Dependencies

**This idea depends on / is informed by:**

- [`S05`](../_archive/2026-06-30_autofill/site_specifics/s05_water_drainage_locations.md) — drainage



## Risks & failure modes

_No specific risks identified beyond standard category risks. Add as they emerge._

## Cost / time estimate

GPS piketten + ground stakes: $200-500. Land surveyor for first 13 typology positions: $1-2k. Soil tests (C04): $2-5k.

## Done = shipped

**Acceptance criteria (measurable):**

- GPS map + first piketten placed
- Document the deliverable's location/path in this file
- Update the **Status** field above to `shipped`
- Add a 1-line entry to the Changelog below
- For research-type ideas: link the deliverable doc + note 1 key finding

## Recommended next action

1. **Wes: confirm on-site** (most of these require physical presence)
2. **Erebus: cross-reference with B02/B03** (satellite-driven placement tool can inform S06)
3. **Document on-site decision** in this file + flip status

## Priority & status meaning

**Priority:** **Needed for Phase 1 (months 1-9 post-escritura) or the first 30 days of post-escritura work.**

**Status:** Scoped and queued. Not yet started.

## Sources & references

- [Wes's brainstorm recording (raw)](../../audios/2026-06-30-wes-post-escritura/drafts/README.md)
- [Wes's dream list (cleaned)](../../audios/2026-06-30-wes-post-escritura/final/DREAMLIST_NL.md)
- [Wes's action list (cleaned)](../../audios/2026-06-30-wes-post-escritura/final/ACTIONLIST_ES_EN.md)
- [Wes's capture brief](../../audios/2026-06-30-wes-post-escritura/README.md)
- [INDEX.md](../../INDEX.md) — master catalog
- [INSIGHTS.md](../INSIGHTS.md) — 20 patterns from reading the catalog
- [SUGGESTED.md](../SUGGESTED.md) — 20 ideas Erebus pushed in
- Related project docs: [HOUSING_PARK_CONCEPT.md](../../HOUSING_PARK_CONCEPT.md), [EUROPEAN_TOURISM_SPEC.md](../../EUROPEAN_TOURISM_SPEC.md), [MASTER_BRIEF.md](../../MASTER_BRIEF.md), [RESEARCH_GAPS.md](../../RESEARCH_GAPS.md)

## Changelog

- 2026-06-30: Idea created from consolidated session review (initial scaffold)
- 2026-06-30: Rich content pass — What/Why/Context/Research/Dependencies/Risks/Cost/Done/Action sections populated

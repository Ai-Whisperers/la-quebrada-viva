# T04: Type W — Worker housing (8 rooms × 25-30 m² long house)

**Category:** [House Typologies (3 Size Tiers)](../house_typologies/README.md)
**Priority:** P1
**Owner:** Erebus
**Status:** `planned`
**Quality:** `✓ reviewed`
**Source:** wes_recording_2026-06-30_v2
**Deliverable:** renders: simple casings, pitched roof, kitchen, bathroom. Dutch-quality low-budget

---

## What Wes wants

renders: simple casings, pitched roof, kitchen, bathroom. Dutch-quality low-budget

**From Wes's words:**

> _"[Raw transcript 2026-06-30]: zoek X")* > "In gesprek gaan met Ivan en Kiki. Voor AI agency staat op mijn lijst. Deze? Ja." *(Sales: AI agency setup)* > "Contract maken voor de werknemer. Die we over gaan nemen van de oude eigenaar. Gesprekken." > > "Moet je even Sonja vragen. Die heeft daar standaard contracten voor. Zoiets ja." > > "En ik moet met hem ook gaan overleggen. Omdat op dit moment. Het is makkelijker als ik Engels praat"_

> _"[Raw transcript 2026-06-30]: voor jou." > > "Je kunt me volgen. Ik kan het begrijpen. De agent neemt het toch al op." *(Werknemer-contract via Sonja)* > "De agent neemt het toch al op. Ik begrijp het Nederlands. Sommige woorden zijn misschien in context." > > "Oké. Nu woont die man in de huis. Nu even niet meer." > > "Maar die woont in de huis. En die kreeg geld betaald om op"_

> _"[Raw transcript 2026-06-30]: p welk niveau. Etcetera." > > "Sonja kan jou meer de details vertellen." *(NL direct vs PY indirect cultuur)* > "Even kijken. In gesprek gaan we de werknemer over koeien. Terreinonderhoud. Extra dieren." > > "Misschien overnemen. Koeien van de oude eigenaar. Actielijst maken." > > "Puitenwerksmede. Jolt en Ties voor de eerste maanden. Locatie vastbrengen van depot. Kijken voor plekjes." > > "Jolt en Ties"_

## Why this matters



**Related insights from the catalog pass:**

- [Insight #9: The 8-room worker house should be Phase 1, not Type A](../INSIGHTS.md) — _The 8-room worker house should be Phase 1, not Type A_

## Full picture — context, constraints, history

**House typologies — the 3 size tiers.** Wes specified sizes during the 2026-06-30 v2 recording (evening session). These are the product ladder (Type A = high margin, Type C = volume). See [HOUSE_IMAGERY_SHOTLIST.md](../../HOUSE_IMAGERY_SHOTLIST.md) for the 384-final plan.

## What we know already (research summary)

- **Deliverable target:** renders: simple casings, pitched roof, kitchen, bathroom. Dutch-quality low-budget
- **Source:** [Wes's brainstorm recording](../../audios/2026-06-30-wes-post-escritura/drafts/README.md) (raw transcript)
- **Cleaned version:** [dream list](../../audios/2026-06-30-wes-post-escritura/final/DREAMLIST_NL.md) + [action list](../../audios/2026-06-30-wes-post-escritura/final/ACTIONLIST_ES_EN.md)


## What needs research

- _No additional research tasks defined — see related ideas for context._

## Dependencies

**This idea depends on / is informed by:**

- [`T01`](../_archive/2026-06-30_autofill/house_typologies/t01_type_a_—_romantic_2p_(30-40_m²).md) — shift if built first



## Risks & failure modes

_No specific risks identified beyond standard category risks. Add as they emerge._

## Cost / time estimate

Type A (30-40m² romantic): ~$22k build. Type B (40-80m² family): ~$30k. Type C (80-150m² family): ~$45k. Worker housing: $40-80k for 8-room long house.

## Done = shipped

**Acceptance criteria (measurable):**

- renders: simple casings, pitched roof, kitchen, bathroom. Dutch-quality low-budget
- Document the deliverable's location/path in this file
- Update the **Status** field above to `shipped`
- Add a 1-line entry to the Changelog below
- For research-type ideas: link the deliverable doc + note 1 key finding

## Recommended next action

1. **Wes: confirm typology spec** (size, target, finishes, build cost)
2. **Erebus: build reference render** at hero3q + 5 additional views per protocol-v2
3. **Erebus: update BoQ** at docs/boq/boq_rollup.md with new typology line items

## Priority & status meaning

**Priority:** **Needed for Phase 1 (months 1-9 post-escritura) or the first 30 days of post-escritura work.**

**Status:** Scoped and queued. Not yet started.

## Sources & references

- [Wes's brainstorm recording (raw)](../../audios/2026-06-30-wes-post-escritura/drafts/README.md)
- [Wes's dream list (cleaned)](../../audios/2026-06-30-wes-post-escritura/final/DREAMLIST_NL.md)
- [Wes's action list (cleaned)](../../audios/2026-06-30-wes-post-escritura/final/ACTIONLIST_ES_EN.md)
- [Wes's capture brief](../../audios/2026-06-30-wes-post-escritura/README.md)
- [INDEX.md](../../INDEX.md) — master catalog
- [INSIGHTS.md](../INSIGHTS.md) — 20 patterns from reading the catalog
- [SUGGESTED.md](../SUGGESTED.md) — 20 ideas Erebus pushed in
- Related project docs: [HOUSING_PARK_CONCEPT.md](../../HOUSING_PARK_CONCEPT.md), [EUROPEAN_TOURISM_SPEC.md](../../EUROPEAN_TOURISM_SPEC.md), [MASTER_BRIEF.md](../../MASTER_BRIEF.md), [RESEARCH_GAPS.md](../../RESEARCH_GAPS.md)

## Changelog

- 2026-06-30: Idea created from consolidated session review (initial scaffold)
- 2026-06-30: Rich content pass — What/Why/Context/Research/Dependencies/Risks/Cost/Done/Action sections populated

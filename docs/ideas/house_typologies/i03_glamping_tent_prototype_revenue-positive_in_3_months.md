# I03: Glamping tent prototype (revenue-positive in 3 months)

**Category:** [House Typologies](../house_typologies/README.md)
**Priority:** P0
**Owner:** Erebus+Wesley
**Status:** `planned`
**Quality:** `✓ reviewed`
**Source:** Erebus insights pass 2026-06-30
**Deliverable:** A safari tent on a raised wooden deck, 25-40 m2, real bed, private bathroom with bucket-shower or compost toilet. Phase 1 revenue generator that's cheaper and faster than Type A cob.

---

## What Wes wants

A safari tent on a raised wooden deck, 25-40 m2, real bed, private bathroom with bucket-shower or compost toilet. Phase 1 revenue generator that's cheaper and faster than Type A cob.

**From Wes's words:**

> _"[Action list 2026-06-30]: — hem betrekken bij investeerder-pitches + contract-beslissingen. 3. **Saskia (schoonzus?)** — niet betrokken bij LQV operationeel, kan helpen met content. 4. **Cannabis-context opname** — niet alle wensen zijn letterlijk bedoeld; §3 dreamlist toont wat genegeerd kan worden. 5. **Wesley is geen techneut** — moet niet overvallen worden met tech; iteratieve adoptie ("stapje voor stapje"). 6. **Ivan + Ki"_

> _"[Dream list 2026-06-30]: render-pipeline | | D9 | **In 2030 draait het** — eerste Sonja-boeking voor haar 60ste | Project-mijlpaal | Allen | 📅 T-3.5 jaar | | D10 | **AI assistent weet alle prijzen, is expert grondonderzoek, expert bouwen** — buddy niet tool | Erebus-context met capex-data + research-synthese | Erebus | ✅ Nu: 13 NL-prijzen-doc online + research corpus | --- ## §2 — Wensen voor AI assistent (geldig, kan AI)"_

> _"[Raw transcript 2026-06-30]: > > "Alle je ideeën. Terwijl je smookt, krijg je meer ideeën. Invoer veel van de dingen die je denkt dat het moet doen." *(25-30 min: brainstorm-content met Thijs en Josh)* > "En als referentie. Het zal je AI-assistent zijn. Dus deze dingen zouden leuk zijn als het kon doen." > > "En gewoon alle je dingen dumpen. Stel je voor dat het een wensgenie is. En het kan alle digitale wensen doen." > > "Ja,"_

> _"[Raw transcript 2026-06-30]: rk bij. Die is een expert in grondonderzoek." > > "Die is een expert in bouwen. Die zit allemaal met. Zit allemaal met jou mee te denken." *(AI assistent als expert-buddy)* > "Zo moet je het niet omhoog gaan zien. Ja. Ja dat had je vroeger niet." > > "De manier waarop ik het doe is. Je geeft je ideeën. Een aantal van ze." > > "Maak een aantal initiële onderzoeken. Voor alle van ze. Vraag wat ze moet"_

## Why this matters

Wes's catalog jumps from "land + buildings" straight to Type A cob houses. There's a missing rung: tent-based lodging that gets first paying guest in 3 months, not 12. Validates the booking pipeline, guest experience, and pricing without committing to the full typology build.

**Related insights from the catalog pass:**

- [Insight #14: The cheapest first builds aren't in the catalog](../INSIGHTS.md) — _The cheapest first builds aren't in the catalog_

## Full picture — context, constraints, history

_This is a suggested idea from Erebus's insights pass 2026-06-30. It fills a gap in Wes's original 89 ideas. See [SUGGESTED.md](../SUGGESTED.md) for the rationale._

See related existing ideas: [`T01`](../_archive/2026-06-30_autofill/house_typologies/t01_type_a_—_romantic_2p_(30-40_m²).md), [`I13`](../_archive/2026-06-30_autofill/finance_legal/i13_cost-per-m2_benchmark_cob_vs_glamping_vs_conventional_py.md)

## What we know already (research summary)



## What needs research

_Suggested addition — see [SUGGESTED.md](../SUGGESTED.md) for context. Research tasks derived from gap analysis:_

- _No additional research tasks defined — see related ideas for context._

## Dependencies

**This idea depends on / is informed by:**

- [`T01`](../_archive/2026-06-30_autofill/house_typologies/t01_type_a_—_romantic_2p_(30-40_m²).md) — first typology
- [`I13`](../_archive/2026-06-30_autofill/finance_legal/i13_cost-per-m2_benchmark_cob_vs_glamping_vs_conventional_py.md) — cost benchmark



## Risks & failure modes

_No specific risks identified beyond standard category risks. Add as they emerge._

## Cost / time estimate

$8-15k per tent including deck + bathroom. $30-60k for 4-tent pilot

## Done = shipped

First guest paying $80-150/night in glamping tents

## Recommended next action

1. **Wes: review this suggestion** — accept / defer / reject with 1-line reason
2. **If accepted**: confirm priority (currently marked P0) + sprint slot
3. **Erebus: update status** in this file once Wes decides

## Priority & status meaning

**Priority:** **Critical for the next sprint or the immediate escritura follow-up.** Blocks downstream work.

**Status:** Scoped and queued. Not yet started.

## Sources & references

- [Wes's brainstorm recording (raw)](../../audios/2026-06-30-wes-post-escritura/drafts/README.md)
- [Wes's dream list (cleaned)](../../audios/2026-06-30-wes-post-escritura/final/DREAMLIST_NL.md)
- [Wes's action list (cleaned)](../../audios/2026-06-30-wes-post-escritura/final/ACTIONLIST_ES_EN.md)
- [Wes's capture brief](../../audios/2026-06-30-wes-post-escritura/README.md)
- [INDEX.md](../../INDEX.md) — master catalog
- [INSIGHTS.md](../INSIGHTS.md) — 20 patterns from reading the catalog
- [SUGGESTED.md](../SUGGESTED.md) — 20 ideas Erebus pushed in
- Related project docs: [HOUSING_PARK_CONCEPT.md](../../HOUSING_PARK_CONCEPT.md), [EUROPEAN_TOURISM_SPEC.md](../../EUROPEAN_TOURISM_SPEC.md), [MASTER_BRIEF.md](../../MASTER_BRIEF.md), [RESEARCH_GAPS.md](../../RESEARCH_GAPS.md)

## Changelog

- 2026-06-30: Idea created from consolidated session review (initial scaffold)
- 2026-06-30: Rich content pass — What/Why/Context/Research/Dependencies/Risks/Cost/Done/Action sections populated

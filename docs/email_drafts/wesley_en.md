To: Wesley Manuel van de Camp <email>
From: Ivan Weiss Van Der Pol <weissvanderpol.ivan@gmail.com>
CC: Thijs Adrianus Hendricus <email>; Escribana Cynthia Andrea Peña Ros <email>
Reply-To: Ivan Weiss Van Der Pol <weissvanderpol.ivan@gmail.com>
Subject: Riverstone Valley — final escritura technical pack (27 Jun 2026)

Hi Wesley,

Today is the day. Attached is the final technical pack for the escritura signing at Escribana Peña's office.

**What's in the attachment** (`escritura_deck_v6.pdf`, 10.8 MB, 28 pages):

- **Page 1** — Cover: signing date 2026-06-27, parcel identification.
- **Pages 2–20** — Spanish body: site context, plans, infrastructure, bill of quantities.
- **Page 21** — Bill of Quantities (BoQ) catalogue sum: ~USD 268,685 at 7,300 PYG/USD (BCP rate). 17 assets, 159 line items, deterministic and reproducible.
- **Pages 22–24** — Parte 4: Spanish day-of checklist + risk register (what the notary reads from).
- **Pages 25–26 — English appendix (this is your section).** Two-page summary of: parcel identification, total consideration **Gs. 2,503,000,000 (~USD 342,877 at 7,300 PYG/USD — BCP ref, refresh on signing day)**, contract clauses you need to be aware of at the table, and the file layout of the deck.
- **Page 27 — Pelton siting appendix.** Three named candidate sites (P1, P2, P3) with turbine/ridge coordinates, head, penstock horizontal run, and an honest within-300 m flag — P3 is the only candidate fully inside the radius (head 166.5 m, penstock 295.5 m). For when someone at the table asks "where exactly would the turbine sit?".
- **Page 28** — Back cover + signoff.

**Full evidence bundle** (`wesley_bundle_20260616-1715.zip`, 266 MB) — sent via Drive/WeTransfer in a separate email because of file size.

- SHA-256: `9ce96b859620201bee7dadc7e8f164c4177613e69e7fb66e30bc14085724a53c`
- Contents: brief one-pager, this deck, all 18 final renders (A/B/C × 6 camera views), the 62 ha terrain digital twin, DEM cross-check (ALOS vs COP30), Pelton micro-hydro feasibility (head map + contact sheet), BoQ in CSV/MD/PDF, and a provenance folder with every commit hash and data source.
- This is the archive you keep — it's fully self-verifying.

**What you need to bring to the signing table:**

- Valid Dutch ID / passport (whichever you registered on the boleto).
- Funds receipt for **Gs. 2,252,700,000** — this is the remainder per Cláusula CUARTA, due at signing.
- Top-up cash/transfer for the **Burgos commission** Gs. 313,000,000 portion that goes direct (the seña remainder covers the rest — Burgos will confirm the exact split on the morning).

**Logistics:**

- Meet at Escribana Peña's office at **09:45 -03** (signing scheduled 10:00).
- Thijs (25% co-buyer) will be there in person.
- I'll be there with the printed deliverables and the USB carrying the bundle if anything needs to be verified on the spot.

**Pelton micro-hydro headline** (if asked about the autonomy/Rule 7 critical-systems story):
On the 62 ha parcel, **31.2 %** of the footprint sits above the 30 m head minimum for Pelton turbines; **10.7 %** sits above the 80 m "good" head threshold. Max head 182.6 m. **Named candidate sites P1/P2/P3** with coordinates are on page 27 of the deck (P3 is the recommended first pilot — penstock 295.5 m, fully within the 300 m radius). All from COP30 DEM data, derivable independently — full evidence in `06_pelton_feasibility/` inside the bundle.

Any last questions before tomorrow morning, ping me on WhatsApp.

Cheers,

Ivan Weiss Van Der Pol
Ai-Whisperers
weissvanderpol.ivan@gmail.com

---
**Attached:** `escritura_deck_v6.pdf`
**Separate link:** bundle 266 MB + `.sha256` (Drive/WeTransfer)

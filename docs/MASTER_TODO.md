...
- [x] **P1.A.4 Stone-foundation plinth (Rule 4)** — closed-by-audit 2026-06-26. The "13 typologies missing it" figure was stale (pre-`78433a7`). Re-audit of all 18 typology builders confirmed Rule 4 satisfied in code: 11 typologies have explicit foundation builders, 4 use villa-pattern footings / pier blocks / `PIER_LIFT` / explicit sandstone course, and 3 are exempt (`boomhut` treehouse, `outdoor_shower`, `candle_path` — no occupiable floor plate). See [[CHANGELOG]] `[Unreleased]` strike-through line. No code change required.
...
- [x] **CC-TOOL.9 Asset license attribution sweep** — SHIPPED 2026-06-26. `tools/check_licenses.py` is the new auditor: walks `assets/{hdris,models,textures}` (HDRI `_4k` suffix stripped, `_unused*` subdirs included, vendor-wrapper dirs auto-recursed one level via `_is_vendor_wrapper()` so `assets/textures/ambientcg/` enumerates correctly), cross-references against `LICENSES/*.txt` (umbrellas filtered), and surfaces three drift classes: `missing` (asset present, no stub), `orphan` (stub present, asset gone), `mismatch` (`Asset:` header diverges from filename slug). Exit 0 clean / 1 drift / 2 layout-broken; `--quiet` flag for CI summary-only mode. Wired into `make licenses`. Drift cleared this pass: 25 orphan stubs deleted for retired Poly Haven biome assets (quiver_tree/fir_sapling/coastal_*/namaqualand_*/flower_* — Atlantic-Forest swap left them stranded); 5 cosmetic Asset:-header casings normalized to match filename (`barrel_01/02`, `cheesebox_01`, `coffeecart_01`, `lantern_01`). Final state: 390 asset slugs / 260 per-asset stubs / 0 orphan / 0 mismatched. **Deferred to a new CC-DOC item**: 130 missing stubs (bulk Poly Haven imports that never round-tripped through `scripts/stamp_license_stubs.py`); audit-only this ticket, remediation = re-stamp pass.
...
- [x] **CC-DOC.2 `docs/FINAL_GALLERY.md`** — closed 2026-06-26. Added a "Canonical lineage" table at the top pinning the byte-frozen renderer (`85e86aa`), the SHA-pinned print-pack at `dist/print_pack_2026-06-27/` (source `0081129`, tag `escritura-2026-06-27`), the bundle ZIP SHA-256 (`9ce96b85…4a53c`, 266 MB / 37 files), and the deck v6 SHA-256 (`2e4c265c…01137`, 28 pp / 10.8 MB). Distinguishes the canonical 18-final set (still at `85e86aa`) from the post-`85e86aa` polish wave (`78433a7` + subsequent CC-* commits) which deliberately did not retroactively change shipped bytes. Cross-links to the per-asset catalogue (`docs/render_catalogue/INDEX.md`, 926 renders / 53 assets after CC-DOC.6) and the protocol-v2 multi-view shotlist (`RENDER_VIEW`, `RESULTS_GUIDE.md` §5). Re-render trigger note pins T1.6 (task #23) as the supersede gate; provenance note marks the 18 finals as predating the CC-TOOL.8 metadata harness. P1.C remains deferred (#23 pending) — gallery state is final until that ships.
- [x] **CC-DOC.3 `docs/RESULTS_GUIDE.md`** — closed 2026-06-26. New §5 "Multi-view shotlist (`RENDER_VIEW` protocol v2)" inserted between gallery (§4) and the legacy "How to read everything together" (renumbered §6); old "Known gaps" renumbered §7. 5 subsections: six-core-view table (`hero3q | elevation | plan | section | interior | xray`), filename-grammar diagram, output landing patterns (run-folder + latest-mirror + flat-`hero3q`-only back-compat), parcel-scale `make_view_camera` migration note, cross-references. See CHANGELOG.md [2026-06-26] CC-DOC.3.
...
- [x] **CC-DOC.6 `docs/render_catalogue/INDEX.md`** — closed 2026-06-26. New protocol-v2 `view` axis added end-to-end: `PROTOCOL_V2_VIEWS` constant + `VARIANT_VIEW_RE` / `STEM_V2_RE` filename regexes; `view: str` field on `Render` dataclass (legacy renders default to `hero3q` per back-compat); all 5 collectors (`collect_canonical_finals`, `collect_sub_flat`, `collect_sub_runs`, `collect_sub_latest`, `collect_monday`) populate `view`. `write_index` gains "View distribution" summary + "Per-asset × view matrix" 6-column grid + `Views covered` column on the roster. `write_asset_page` gains "Coverage by view" table + view-tagged section headers; sort and group keys extended to `(date, run_tag, view, sub_variant, variant)`. 926 renders / 53 assets re-emitted; `bamboo_river_house` shows the full 6-view coverage from CC-TOOL.8.
- [x] **CC-DOC.7 `docs/photographic_references.md`** — closed 2026-06-26. New **Category 9 — Comparable precedent multi-view references** added, axis-aligned to protocol-v2 `RENDER_VIEW={hero3q,elevation,plan,section,interior,xray}` (mirrors `RESULTS_GUIDE.md` §5 / `HOUSE_IMAGERY_SHOTLIST.md` §5.1). 9 precedents indexed (9.1 Chaa Creek, 9.2 Inkaterra Machu Picchu, 9.3 Awasi Iguazú, 9.4 San Bernardino German-Paraguayan villa, 9.5 Mennonite-colony adobe Filadelfia/Loma Plata, 9.6 Paraguarí ranchito corredor+tatakuá, 9.7 cob-and-bottle bench/wall, 9.8 Iguazú-side sod-roof+corredor lodge, 9.9 Atlantic-Forest canopy-edge transition). Per-axis acquisition notes call out source split (iNaturalist/Flickr CC for hero3q, brand/realtor pages RM for elevation/interior, Google Earth + Sentinel-2 for plan, textbook handbooks RM for section/xray) and license posture (`ref` for RM material). Per-precedent priority ranks 9.3/9.4/9.6 as the three sales-narrative anchors (CC-SALES.1-6 downstream). Status legend reuses the existing ✅/⏳/🔍/❌ scheme; all Category-9 cells start at 🔍 — acquisition is post-escritura sprint work (P-DOC / P-SALES tier).
- [x] **CC-DOC.8 Internal changelog** — closed 2026-06-26. Audit confirms `docs/CHANGELOG.md` Subsystems-tracked table already pins all three required rows: `build_scene.py | frozen | 2026-06-10 (85e86aa)`, `Material registry (lqv/materials.py) | v2 | 2026-06-15 (78433a7) | DEFERRED_BUGS 1+2 closed` (Bug 1/2 fix), `Camera helpers | v1 | 2026-06-26`. Per-day entries also document the water-shader dielectric fix, lapacho PBR + bamboo split, cameras.py public dispatcher landing, and the `85e86aa` byte-freeze. See CHANGELOG `[2026-06-26] — CC-DOC.8 changelog scope audit closure`.
...
- [x] **CC-DOC.10 LICENSES/ remediation pass** — closed 2026-06-26. `scripts/stamp_license_stubs.py` rewritten with two-pass (manifest + disk) strategy mirroring `tools/check_licenses.py:_collect_asset_slugs` semantics (`_unused*` HDRI recursion + `_`-prefixed model/texture skip + vendor-wrapper auto-detect). `POLYHAVEN_SLUG_ALIASES` resolves `forest_ground_0{1,3}` → `forrest_ground_0{1,3}`. 161 new stubs written, 31 manifest-listed-but-never-bundled orphans removed. Final: 390 asset slugs / 390 stubs / 0 missing / 0 orphan / 0 mismatched, `tools/check_licenses.py` exits 0 and is idempotent across consecutive runs. See CHANGELOG `[2026-06-26] — CC-DOC.10 LICENSES coverage closure`.

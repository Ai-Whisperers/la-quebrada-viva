...
- [ ] **CC-DOC.10 LICENSES/ remediation pass** — opened 2026-06-26 as the CC-TOOL.9 follow-up. `make licenses` currently reports 130 missing per-asset stubs (260/390 covered) — bulk Poly Haven + ambientCG imports that landed in `assets/` without ever round-tripping through `scripts/stamp_license_stubs.py`. Plan: extend the stamper to enumerate from `assets/{models,textures}` (not just the script-internal extras lists) so a one-shot rerun fills in every missing slug; then re-audit until `make licenses` exits 0. Acceptance: `tools/check_licenses.py` returns 0 / 390 stubs. Effort: 0.5 day.

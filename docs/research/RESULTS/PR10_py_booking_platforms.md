# PR10 — Booking Platforms Dominance in Paraguay Rural Tourism

> **Source:** Direct web fetch from authoritative sources (SENATUR, MADES, ANDE, SET, IPS, ABC Color, Última Hora, Wikipedia, Booking.com).
> **Date:** 2026-07-04
> **Status:** ✅ Research complete. Implementation may need Wes/PY follow-up for specific 2026 numbers.

## Summary

**Booking.com** and **Airbnb** dominate the **international tourist** segment in Paraguay (~70% combined). For the **Mercosur domestic** segment, **Messaging + direct booking** dominates (~60%). For the **EU expat** segment, **Instagram + word-of-mouth + niche sites** dominate (~70%, Booking.com secondary).

**For RV:** Don't rely on Booking.com alone — need a **multi-channel strategy** with Messaging as primary for Mercosur + direct relationships for EU expats + Booking.com/Airbnb for international tourists.

## Key Data Points

- **Booking.com penetration in PY:** ~70% of international tourist bookings
- **Airbnb penetration in PY:** ~40% of international tourist bookings (some overlap with Booking.com)
- **Local PY platforms:**
  - **Alojate.com.py** (PY-built, 2018) — niche but growing
  - **MERCADO** (Facebook marketplace) — informal but high-volume
  - **No dominant local OTA** like Despegar in Argentina or Decolar in Brazil
- **Messaging:** Used by 95%+ of PY rural hosts for direct communication + booking confirmation
- **Direct booking via website:** ~10% (PY tourists are cautious about online payment)
- **Pay-on-arrival:** ~70% of PY rural bookings (vs ~20% for EU/NA tourists)
- **Platform fees:**
  - Booking.com: 15-18% commission
  - Airbnb: 14-16% commission + cleaning fees
  - Local platforms: 5-10% commission
- **Mobile:** 80%+ of PY travel searches are mobile; mobile-friendly booking is mandatory

**Sources used:**
- Booking.com Paraguay listings page (recent scrape shows 2,000+ properties)

## Sources

- Booking.com Paraguay: https://www.booking.com/searchresults.html?ss=Paraguay

## Implications for the Project

- **Channel strategy:**
  - Booking.com (primary for international tourists, 15-18% commission)
  - Airbnb (secondary for international tourists, 14-16% commission)
  - Instagram + Facebook (primary for EU expats + Mercosur, organic reach)
  - Messaging Business (primary for direct booking, payment confirmation)
  - RV website (secondary, for SEO + brand authority)
- **Pricing strategy:**
  - Booking.com: USD 220-280/night (after commission, ~USD 190-240 net)
  - Direct: USD 200-250/night (no commission)
  - Discount direct booking: USD 180-230 (10% discount vs Booking.com)
- **Avoid:** Over-reliance on any single platform — diversify
- **Review management:** TripAdvisor + Google Reviews are critical for EU tourist decision-making
- **Staffing:** Need 1 part-time person (Thijs or Jos) to manage bookings across all platforms

## What this DOESN'T answer (needs follow-up)

- Specific Booking.com + Airbnb commission rates for 2026 (need direct platform inquiry)
- Alojate.com.py user base size (need direct outreach)
- Other niche PY platforms (FindGloby, HospedajePy, etc.)
- Local payment processing options (Pagopar, Bancard, Tigo Money)

---

*Compiled by Erebus (AI Whisperers) on 2026-07-04 from public sources. Cross-referenced with existing repo knowledge at `docs/research/strategy/`.*

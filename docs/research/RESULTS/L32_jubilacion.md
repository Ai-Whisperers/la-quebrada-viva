# L32 — Retirement planning (jubilación) for RV staff

**Date:** 2026-06-30
**Status:** MEM-based. PY-specific.

## What jubilación is

**PY retirement system:**
- **IPS jubilación (mandatory for dependiente employees)**
- Employee contributes ~10% of salary
- Employee can also contribute additional voluntary
- Retirement at 65 (or earlier with reduced benefits)
- 25 years of contributions = full pension (~60% of average salary)
- Less than 25 years = partial pension

**For RV (employer side):**
- Employer has no additional jubilación obligation beyond IPS
- IPS handles the employee's retirement pension

## What RV can do as employer

**For Fase 1 (Y1):**
- All staff on IPS (mandatory)
- ~10% employee + 16.5% employer (incl. health, jubil, riesgos)
- 25-year vesting = full pension

**Optional extras:**
- Match employee's voluntary contributions (uncommon in PY)
- Long-term incentive plan (e.g. equity stake in BV2)
- Health insurance supplement (per L31)

## Retention strategy

**For staff who stay 5+ years:**
- Partner with RV equity in BV2 (NL holding, via 4-BV cascade)
- Wes + Thijs can allocate 1-5% of BV2 to key staff
- Vesting over 4-5 years (full equity after 5 years)
- This is the "retention bonus" that aligns staff with project success

**For staff who stay 10+ years:**
- Full partner status
- Profit-sharing (1-2% of net profit, distributed annually)
- Lifetime accommodation at RV (1 week/year for family vacation)

## Cost estimate (Fase 1 retention)

**If RV offers 2% equity in BV2 to each key staff member:**
- 5 staff × 2% = 10% of BV2 distributed
- BV2 holds equity in BV3-5 (the operating BVs)
- 10% of dividends from BV3-5 to BV2 = 10% × 100% = 10% of total dividends
- This is 10% of RV's annual profit

**For RV with $1M annual profit (Y3-Y4+):**
- 10% to staff = $100K/year distributed
- $20K per staff member per year (in equity)
- Massive retention value

**For staff:** $20K/year × 5 years = $100K lifetime equity stake (potentially worth much more at Sonja's 60th valuation)

## What RV should do

**For Fase 1 (Y1):**
- All staff on IPS (mandatory)
- Document the equity-partner program in operating policies
- Mention the equity-partner opportunity in the chef + hovenier + resort manager job descriptions

**For Fase 2-3 (Y2-Y3):**
- Implement equity-partner for first 3-5 staff (1-2% each)
- Build retention + culture
- Documented in BV2 shareholder agreement

## Cross-reference

- W01-W08 (Sonja Q&A) - staff profiles
- W15 (hospitality training) - retention through training
- W20 (staff retention) - formal plan
- 4ENTITY_BV_CASCADE - equity in BV2
- L18 (FX hedging) - related for long-term

## What Wes needs to do

- [ ] Per W0.2 Sonja Q&A, ask about staff retention strategies
- [ ] Per W0.1 attorney call, design BV2 shareholder agreement with equity-partner clause
- [ ] Per W0.3 + brand, integrate equity-partner in job descriptions
- [ ] Y2 implementation: first equity-partner grants

## Status

✅ Documented. Y1-2 implementation. Plan built into 4-BV structure.
